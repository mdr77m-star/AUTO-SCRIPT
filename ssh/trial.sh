#!/bin/bash
# SENSI — Trial SSH Account (SlowDNS + UDP Custom + WireGuard info)
...
sleep 0.5
clear
useradd -e "$(date -d "$hari days" +"%Y-%m-%d")" -s /bin/false -M "$Login" 2>/dev/null
exp=$(chage -l "$Login" 2>/dev/null | grep "Account expires" | awk -F": " '{print $2}')
echo -e "$Pass\n$Pass\n" | passwd "$Login" &> /dev/null

# ── Online connections for this trial user ────────────
...
echo -e "\E[0;41;36m            SENSI TRIAL SSH              \E[0m"
echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━\033[0m"
